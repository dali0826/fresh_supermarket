...
from goods.forms import forms, GoodsForm
...
def goods_list(request):
    if request.method=='GET':
        ...
        return render(request,'goods_list.html',{'page':page,
                                                 'category_types':category_types})
...
def goods_add(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            #字段验证不通过，返回页面，并在页面中使用 form.errors拿到错误信息
...
def goods_desc(request,num):
    if request.method=='GET':
        goods=Goods.objects.get(pk=num)
        return render(request,'goods_desc.html',{'goods':goods})
    if request.method=='POST':
        #保存商品的描述信息
        content=request.POST.get('content')
        Goods.objects.filter(pk=num).update(goods_desc=content)
        return HttpResponseRedirect(reverse('goods:goods_list'))