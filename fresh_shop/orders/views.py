...
def place_order(request):
    if request.method=='GET':
        ...
        #获取当前勾选的商品用于下单
        cart_goods=ShoppingCart.objects.filter(user_id='',is_select='True')
        #在购物车对象上绑定total_price字段，用于存商品的总价
    ...
    if request.method=='POST':
        ...
        user_id=request.session['user_id']
        #获取购物车中当前登录用户勾选的商品
        carts=ShoppingCart.objects.filter(user_id=user_id,is_select=True).first()
        #订单货号
